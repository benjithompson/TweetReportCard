...
def init_stats():
    ...
    return {
        'flesch_ease': 0.0,  # [0-100] Lower is more challenging to read. Wiki: Avg=49.13, Politicians = 46.93
...
def get_msg_stats(msg):
    ...
    s['totalavg'] = (((1-s['flesch_ease'])/100.0) + (s['flesch_grade']/20.0) + (s['dalechall']/10.0) + (s['ari']/20.0) + (s['colemanliau']/30.0) + (s['linsear']/20.0) + (s['smog']/240.0))/7.0
    return s

def clean_text(text):
    """removes symbols"""
    # text = re.sub('\B.co/\w\w+', ' ', text) # remove .co/
    text = re.sub('\B#\w\w+', ' ', text) # remove hashtags
    text = re.sub('\B@\w\w+', ' ', text) # remove tag
    text = re.sub('https?://.*[\r\n]*', ' ', text) #remove urls
    # text = re.sub('[^a-zA-Z0-9, .\']', '', text).lower() #remove symbols and lowercases
    return text
